...
try:
    ...
    from src.agents.agentic_risk_score_agent import (
        create_agent_graph,
        MLModelWrapper as AgenticMLModel,
        AgentState
    )
    logger.info("✓ Agentic components imported")
    AGENTIC_AVAILABLE = True
except ImportError as e:
    logger.warning(f"⚠ Agentic components not available: {e}")
...
class SubtypeModelManager:
    ...
    def predict(self, biomarker_data: dict) -> dict:
        ...
        for feat in self.feature_names:
            value = biomarker_data.get(feat.lower())
            if value is None:
                value = np.nan
            feature_values.append(value)
...
async def lifespan(app: FastAPI):
    ...
    global risk_agent_graph, risk_ml_model, subtype_model_manager
    ...
    # Initialize Agentic Graph (if available)
    try:
        if AGENTIC_AVAILABLE:
            risk_agent_graph = create_agent_graph()
            logger.info("✓ Risk Score Agentic Agent initialized")
        else:
            logger.warning("⚠ Agentic components not available")
    except Exception as e:
        logger.error(f"✗ Failed to initialize agentic agent: {e}")
        risk_agent_graph = None
    
    logger.info("="*70)
    logger.info(f"Risk ML Model: {'✓' if risk_ml_model else '✗'}")
    logger.info(f"Subtype Model: {'✓' if subtype_model_manager else '✗'}")
    logger.info(f"Agentic Agent: {'✓' if risk_agent_graph else '✗'}")
    logger.info("="*70)
...
class SubtypeBiomarkerData(BaseModel):
    """Input model for subtype classification"""
    ascl1: float = Field(..., ge=0, le=20)
    neurod1: float = Field(..., ge=0, le=20)
    pou2f3: float = Field(..., ge=0, le=20)
    yap1: float = Field(..., ge=0, le=20)
    tp53: float = Field(..., ge=0, le=20)
    rb1: float = Field(..., ge=0, le=20)
    myc: float = Field(..., ge=0, le=20)
    mycl: float = Field(..., ge=0, le=20)
    mycn: float = Field(..., ge=0, le=20)
    
    class Config:
        json_schema_extra = {
            "example": {
                "ascl1": 12.5, "neurod1": 5.0, "pou2f3": 4.0,
                "yap1": 5.5, "tp53": 11.0, "rb1": 4.5,
                "myc": 8.0, "mycl": 9.5, "mycn": 6.0
            }
        }
...
""",
        ...
        for msg in final_state.get('messages', []):
            ...
            # Last message with structured report is the clinical report
            if "RISK ASSESSMENT" in content or len(messages_content) == len(final_state['messages']):
